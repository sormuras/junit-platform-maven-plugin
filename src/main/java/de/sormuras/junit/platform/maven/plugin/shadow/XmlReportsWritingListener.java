...
/**
 * {@link TestExecutionListener} that writes a separate XML report for each root in the {@link
 * TestPlan}.
 *
 * <p>Note that the generated XML format is compatible with the de facto standard for JUnit 4 based
 * test reports that was made popular by the Ant build system.
 */
public class XmlReportsWritingListener implements TestExecutionListener {
  ...
  public XmlReportsWritingListener(Path reportsDir, BiConsumer<String, Throwable> log) {
    this(reportsDir, log, Clock.systemDefaultZone());
  }

  private XmlReportsWritingListener(
      Path reportsDir, BiConsumer<String, Throwable> log, Clock clock) {
