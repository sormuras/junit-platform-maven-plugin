...
/**
 * {@link Extension} for usage with JUnit Jupiter. This is just a wrapper for an embedded {@link
 * AbstractMojoTestCase}, so all <tt>protected</tt> methods of the TestCase are exhibited as
 * <tt>public</tt> in the extension. You may annotate single tests methods with {@link WithoutMojo}
 * to prevent the extension from firing.
 *
 * @author Mirko Friedenhagen
 * @version $Id$
 * @since 2.2
 */
public class MojoExtension implements Extension, BeforeEachCallback {
  ...
  public MojoExtension(AbstractMojoTestCase testCase) {
    this.testCase = testCase;
  }

  /**
   * May be overridden in the implementation to do stuff <em>after</em> the embedded test case is
   * set up but <em>before</em> the current test is actually run.
   *
   * @throws Exception
   */
   ...
   *
   * <p>NOTE: the caller is responsible for casting to to what the desired type is.
   *
   * @param object
   * @param variable
  ...
  /**
   * Convenience method to obtain all variables and values from the mojo (including its
   * superclasses)
   *
   * <p>Note: the values in the map are of type Object so the caller is responsible for casting to
   * desired types.
   *
   * @param object
  ...
  public void beforeEach(ExtensionContext extensionContext) throws Exception {
    if (extensionContext.getRequiredTestMethod().getAnnotation(WithoutMojo.class) != null) {
      return;
    }
    testCase.setUp();
    before();
  ...
  public MavenProject readMavenProject(File basedir) throws Exception {
    ...
    Objects.requireNonNull(project); // Assert.assertNotNull( project );
    return project;
  }
  ...
  public void executeMojo(
      MavenSession session, MavenProject project, String goal, Xpp3Dom... parameters)
      throws Exception {
    MojoExecution execution = newMojoExecution(goal);
    if (parameters != null) {
  ...
  public void executeMojo(MavenSession session, MavenProject project, MojoExecution execution)
    ...
    try {
      ...
      try {
        ...
        for (MojoExecutionListener listener :
            getContainer().lookupList(MojoExecutionListener.class)) {
